...
export async function createKlingJob(params: KlingGenerateParams): Promise<KlingJob> {
  ...
  return { jobId: data.data?.task_id, status: 'pending' }
}
...
export async function checkKlingJob(jobId: string): Promise<KlingJob> {
  ...
  const statusMap: Record<string, KlingJob['status']> = {
    submitted: 'pending', processing: 'processing', succeed: 'completed', failed: 'failed',
  }
  return { jobId, status: statusMap[task.task_status] || 'processing', videoUrl: task.task_result?.videos?.[0]?.url, cost: 0.14 }
}
