...
export default function AdminPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: allProfiles } = await supabase.from('profiles').select('*, plans(name, price_monthly, credits_videos, credits_ebooks, is_unlimited)').order('created_at', { ascending: false })
      if (allProfiles) {
